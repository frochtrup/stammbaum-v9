...
//   (b) EDITIERT ⇒ NAME/GIVN/SURN kommen konsistent in der Datei an. Ein bewusster
//       Nutzer-Edit ist keine automatische Änderung; LP-1 schützt vor Letzterem, nicht
//       vor Ersterem (ADR-v9-81-Lehre, ADR-v9-112 §Diskussion).
...
describe('(b) editiert: der Nutzer-Edit kommt konsistent in der Datei an', () => {
  it('Nachname-Umbenennung erzeugt NAME, GIVN und SURN im Einklang', () => {
    const doc = parseGedcom(UNTAGGED);
    ...
    expect(logical).toContain('2 GIVN Theodor Hermann');
    expect(logical).toContain('2 SURN Zurloh-Meyer');
