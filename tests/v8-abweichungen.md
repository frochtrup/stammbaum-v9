...
| DEV-07 | GRAMPS-Export, Evidenz-Bewertung am `<citation>` (BL-83) | v8 schreibt die vier Achsen als `<attribute type="_STYP\|_INFO\|_EVID\|_INFM" value="…"/>` in das `<citation>`-Element (`gramps-writer.js:625-632`) und liest sie ebenso zurück (`_GR_EVAL_ATTR`). Das ist **DTD-widrig**: `grampsxml.dtd` 1.7.2 definiert `citation ((daterange\|datespan\|dateval\|datestr)?, page?, confidence, noteref*, objref*, srcattribute*, sourceref, tagref*)` — `<attribute>` kommt darin nicht vor (gegen die öffentliche DTD geprüft, nicht gegen die Bestandsdatei). GRAMPS' Importer kennt für Zitat-Attribute nur `srcattribute`; ein `<attribute>` im `<citation>` fällt dort still weg | v9 **schreibt** `<srcattribute type="_STYP" value="…"/>` an der DTD-Position (nach `objref`, vor `sourceref`) — die Form, die auch das echte GRAMPS 6.0.6 in der Realdaten-Fixture verwendet (`<srcattribute type="EVEN" value="CHR"/>`). v9 **liest beide** Formen, damit von v8 geschriebene Dateien ihre Bewertung nicht verlieren; beim ersten Schreiben eines geänderten Zitats wird die Altform entfernt (sonst stünde sie neben der neuen Zeile → `_REPO_MODELLED`-Doppelschreibung, Spec 13 §2.3). Fremde `<attribute>`/`<srcattribute>` bleiben unangetastet (INV-PT) | `bug-fix` | `tests/roundtrip/eval-roundtrip.test.ts` › „v8-Altform `<attribute type=\"_STYP\">` wird ebenfalls gelesen (Register DEV-07)" + „Modell-Edit schreibt die Achsen als `<srcattribute>` vor `<sourceref>`" |
...
- **GED7-`1 SCHMA`-Block fehlt in v9 (gefunden 2026-07-31 beim Bau von BL-83).** Das
  Orakel schreibt im GED7-Modus einen vollständigen Schema-Block in den HEAD
  (`gedcom-writer.js` `_g7WriteSchma`, Zeile 6–15): `1 SCHMA` + je `2 TAG _X
  https://github.com/frochtrup/Stammbaum/ext/_X` für **29** Extension-Tags — darunter
  `_EVAL`/`_STYP`/`_INFO`/`_EVID`/`_INFM`, aber eben auch `_TASK`/`_RLOG`/`_HYPO`/`_UID`/
  `_RUFNAME`/`_FREL`/`_MREL`/`_SCBK`/… Spec 13 §4 nennt SCHMA in der GED7-Tabelle; v9
  emittiert bis heute **keinen** SCHMA-Block (`grep SCHMA core/` → nur ein Kommentar in
  `ged7-adapter.ts`). Das ist bewusst NICHT im Rahmen von BL-83 nachgezogen worden: die
  Lücke betrifft ALLE `_`-Tags, nicht die fünf der Bewertung — ein Block nur für `_EVAL`
  wäre ein halber Mechanismus. Zu entscheiden ist außerdem, ob die Liste (wie in v8)
  statisch ist oder aus den tatsächlich geschriebenen Tags abgeleitet wird; und der Block
  ändert die GED7-HEAD-Bytes. Eigener Bauabschnitt + eigener DEV-Eintrag.
