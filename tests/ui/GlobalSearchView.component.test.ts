...
function renderView(db: ReturnType<typeof makeDatabase>) {
  ...
  const utils = render(GlobalSearchView, {
    props: { appState, onNavigateToPerson, onNavigateToFamily, onNavigateToSource, onNavigateToPlace },
  });

  return { ...utils, onNavigateToPerson, onNavigateToFamily, onNavigateToSource, onNavigateToPlace };
}
...
describe('GlobalSearchView — Eingabe, Mindestlänge, gruppierte Ergebnisse', () => {
  ...
  it('gruppiert Treffer nach Personen/Familien/Quellen/Orte', async () => {
    renderView(seedDb());
